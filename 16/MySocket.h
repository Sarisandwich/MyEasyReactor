...
{
private:
    const int _fd;      //存放fd，构造函数传入
    std::string _ip;    //存放ip，如果是listenfd，存放服务器监听的ip，如果是clientfd，存放对端的ip
    uint16_t _port;     //存放port，如果是listenfd，存放服务器监听的port，如果是clientfd，存放对端的port
public:
